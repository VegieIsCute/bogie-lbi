...
namespace EyeStep
{
	...
	namespace util
	{
		...
		uint32_t GetFirstCall(uint32_t address)
		{
			uint32_t at = address;
			uint32_t func_end = nextPrologue(at);
	...
	void function_info::analyze(uint32_t func)
	{
		...
		/*
		// Example of a __fastcall:
		push ebp
		mov  ebp, esp
		sub  esp, 10h
		push esi
		mov  esi, ecx // <------- We can conclude from this, that it's a thiscall.
		push edi                  but let's keep our eyes open
		test edx, edx // <------- This does not determine whether it's a fastcall
		jle  short loc_???????
		mov  eax, [esi+14h]
		mov  edi, offset unk_???????
		shl  edx, 4 // <--------- Here however, edx's value is MODIFIED, but NOT
		add  edx, 0FFFFFFF0h      directly SET to a specific value. It's a fastcall.
		add  eax, edx

		// If EDX was set to a specific value, before it is being modified or
		// accessed by any other instruction, then it would have absolutely
		// NO purpose of being passed into the function, leaving it as a thiscall
		*/
		/*
		SOLUTIONS:

		if ECX or EDX is found in the second operand at any moment then
		we will set conv to c_thiscall, or to c_fastcall

		but we will NOT DO THIS if ECX or EDX were pushed prior to this,
		or are being used in both operands of an instruction.
		We will NOT set it to c_thiscall or c_fastcall if it's being
		used in the first operand of any of the following instructions:
		mov,
		movq,
		cmp,
		...
		(instructions that directly set the register's value)

		Although this isn't full-proof, it succeeds in the majority
		of what is generated by the compiler
		*/
		std::string opcode = "";
		...
		while (at < func_end)
		{
			...
			// Or directly sets the register's value
			// as opposed to accessing or modifying it.
			if (opcode.find("push ") != std::string::npos
				|| opcode.find("mov") != std::string::npos // ALL `mov` instructions
				|| opcode.find("lea ") != std::string::npos
				|| opcode.find("cmp ") != std::string::npos
				|| opcode.find("xor ") != std::string::npos // commonly generated
				|| opcode.find("or ") != std::string::npos // commonly generated
				|| opcode.find("and ") != std::string::npos // commonly generated
				|| opcode.find("sar ") != std::string::npos // commonly generated
				|| opcode.find("shr ") != std::string::npos // commonly generated
				) {
				// if it looks like `xor ecx, ecx` we must completely ignore.
				// Also, edx_set should not be set to true if it looks like:
				// xor ecx,[ecx+4] because this implies it may be a thiscall
				bool invalid_ecx = (src.reg[0] == R32_ECX && dest.reg[0] == R32_ECX);
				if (invalid_ecx)
				{
					if (dest.imm8 > 0 || dest.imm32 > 0)
					{
						invalid_ecx = false;
					}
				}

				// if it looks like `xor edx, edx` we must completely ignore.
				// Also, edx_set should not be set to true if it looks like:
				// xor edx,[edx+4] because this implies it may be a fastcall
				bool invalid_edx = (src.reg[0] == R32_EDX && dest.reg[0] == R32_EDX);
				if (invalid_edx)
				{
					if (dest.imm8 > 0 || dest.imm32 > 0)
					{
				...
				if (src.reg[0] == R32_ECX)
				{
					if (!invalid_ecx)
					{
						ecx_set = TRUE; // ECX HAS been directly set, meaning it cannot be a thiscall
					}
				}
				else if (src.reg[0] == R32_EDX)
				{
					if (!invalid_edx)
					{
						edx_set = TRUE; // EDX HAS been directly set, meaning it cannot be a fastcall
					}
				}
			}
			...
			if (opcode.find("pop ecx") != std::string::npos)
			{
				ecx_set = FALSE;
			}
			else if (opcode.find("pop edx") != std::string::npos)
			{
				edx_set = FALSE;
			}
			...
			if (!edx_set)
			{
				if (dest.flags & OP_R32 && src.reg[0] != R32_EDX && dest.reg[0] == R32_EDX)
				{
					convention = c_fastcall;
				}
				// Unless it looks more like:
				// `test [edx+4], edx`, or:
				// `test edx, [edx+4]`
				/*else if (src.imm8 > 0 || src.imm32 > 0 || dest.imm8 > 0 || dest.imm32 > 0)
				{
					convention = c_fastcall;
				}*/
			}

			if (!ecx_set && convention != c_fastcall) // EDX could get used before ECX
			{
				if (dest.flags & OP_R32 && src.reg[0] != R32_ECX && dest.reg[0] == R32_ECX)
				{
					convention = c_thiscall;
				}
				/*else if (src.imm8 > 0 || src.imm32 > 0 || dest.imm8 > 0 || dest.imm32 > 0)
			...
			// Append args used in the source operand to the table, for psuedocode
			...
			// Append args used in the destination operand to the table, for psuedocode
